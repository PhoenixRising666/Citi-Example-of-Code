...
class LivePersonSDK : NSObject {
  ...
  public let account : String = "14800077"
  ...
  /// AppInstall Id or LiveEngage App Key
  public let appInstallID : String? = nil
  /// Campaing Info
  private var campaignInfo: LPCampaignInfo?
  /// Get Engagement Callback Declaration
  typealias getEngagementCB = (_ campaign : LPCampaignInfo?, _ error : Error?) -> Void
  /// Send SDEs Callback Declaration
  typealias sendSDEsCB = (_ campaign : LPSendSDEResponse?, _ error : Error?) -> Void
  /// Logout Callback Declaration
  typealias logoutCB = (_ loggedOut : Bool?, _ error : Error?) -> Void
  ...
  // MARK: - LPMessagingSDK Init Methods
  
  /// Will init Singleton with Account Number
  ///
  /// - Parameter brandId: Account Number
  public func initSDK(_ brandID : String, appInstallId : String?) {
    //
    do {
      // Unwrap AppInstallationId
      if let appId = appInstallId  {
        // Create MonitoringParams
        let monitoringParams = LPMonitoringInitParams(appInstallID: appId)
        // Init LPMessagingSDK with Monitoring Features
        try LPMessagingSDK.instance.initialize(self.account, monitoringInitParams: monitoringParams)
      } else {
        // Init LPMessagingSDK
  ...
  public func initLogger(_ level : LogLevel = .debug){
    // Logger
    LPMessagingSDK.instance.subscribeLogEvents(level) { (log) -> () in
      // Logger Trace
  ...
  ///
  /// - Parameter completion: Logout Completion Block (Bool?, Error?)
  public func logout(_ completion : @escaping logoutCB ){
    // Logout SDK
  ...
  public func showConversation(_ viewController : UIViewController){
    // Check if ConversationQuery has been set
    if self.conversationQuery != nil {
      // Get new ConversationViewParams
      let conversationViewParams = LPConversationViewParams(conversationQuery: self.conversationQuery!, containerViewController: viewController,
        isViewOnly: false)
      //
      let authParams = LPAuthenticationParams(authenticationCode: nil, jwt: nil, redirectURI: nil, certPinningPublicKeys: nil, authenticationType: .signup)
      // Show Conversation - Authenticathed
      LPMessagingSDK.instance.showConversation(conversationViewParams, authenticationParams: authParams)
    }
  }
  ...
  public func clearConversation() -> Bool {
    ...
    do{
      ...
      // Print Error
      print("Couldn't clear history error: \(error.localizedDescription)")
      // Return false if conversation was not Clear
      return false
    }
  }
  ...
  public func customizeMessagingScreen () {
    ...
    configuration.dateSeparatorFontSize = UIFont.TextStyle.footnote
  ...
  private func customizeConversation(_ config : LPConfig){
    ...
    config.conversationSeparatorFontSize = UIFont.TextStyle.footnote
